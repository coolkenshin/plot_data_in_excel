...
DBG_LEVEL2 = 2
DBG_LEVEL3 = 3
g_nfs_breakdown_one_file_results = []
...
def _get_row_dict_values(df, column_name_dict, start_row, end_row):
    mydict = column_name_dict.copy()
    mydict['date_time'] = df.iloc[start_row, 0] + ' ' + df.iloc[start_row, 1]
    print(mydict['date_time'])
    ...
    if mydict['total_ops_num'] != 0 and start_row != end_row:
        ...
        newdf = df.loc[start_row+1:end_row-1, [1, 0]]
...
def collect_rows(row_result):
    global g_nfs_breakdown_one_file_results
    g_nfs_breakdown_one_file_results.extend(row_result)
...
9               1       create    NaN    NaN    NaN
        ...
        df_one_file = pd.DataFrame(columns=column_name_dict.keys())
        global g_nfs_breakdown_one_file_results
        g_nfs_breakdown_one_file_results = []
        ...
        while index < len(non_na_index_list):
            ...
            pool.apply_async(_get_row_dict_values, args=(df, column_name_dict, elem, next_elem), callback=collect_rows)
            d_print(elem, DBG_LEVEL3)
            index += interval

        pool.close()
        pool.join()
        df_one_file = pd.DataFrame(g_nfs_breakdown_one_file_results)
        df_one_file.columns = column_name_dict.keys()
        d_print(df_one_file, DBG_LEVEL3)
        df_cat = df_cat.append(df_one_file)
