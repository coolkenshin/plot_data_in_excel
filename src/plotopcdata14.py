...
def _get_nfs_generic_breakdown_rows_slice_dict_values(raw_df, column_name_dict, non_na_index_list):
...
def collect_nfs_generic_breakdown_rows(slice_result):
...
def analyze_nfs_generic_breakdown_file_list(file_name_list, column_name_dict, interval=3600):
    ...
    for i in range(slices):
        ...
        pool.apply_async(_get_nfs_generic_breakdown_rows_slice_dict_values,
                         args=(raw_df, column_name_dict, non_na_index_list_slice),
                         callback=collect_nfs_generic_breakdown_rows)
    if slices_last != 0:
...
def plot_nfs3_ops_op(fn):
    ...
    if os.path.isfile(fn_csv):
        ...
        df = read_df_from_csv(fn_csv)
    else:
        file_name_list = get_file_list_in_dir(g_one_storage_data_dir_name, fn)
        df = analyze_nfs_generic_breakdown_file_list(
            file_name_list, column_name_dict, interval=600)
        df = compute_average_by_date_group(df, column_name_dict)
        write_df_to_csv(fn_csv, df)
    print(df)
...
if __name__ == "__main__":
    ...
    #g_one_storage_data_dir_name = '/Users/mmyu/Tools/eclipse463/workspace/plot_data_in_excel/rawdata2/cfeis01nas11_analytics/cfeis01nas112017-11-03_18_49_03_UTC'
    g_excel_file_name = os.path.basename(
        g_one_storage_data_dir_name).split('_')[0] + ".xlsx"
    """ Create a Pandas Excel g_excel_writer using XlsxWriter as the engine """
    g_excel_writer = pd.ExcelWriter(g_excel_file_name, engine='xlsxwriter')
