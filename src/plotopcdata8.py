...
def draw_nfs_combined_chart2(df, chart_sheet_name, data_sheet_name, chart_title, x_scale=2.5, y_scale=2.5):
    df.to_excel(g_excel_writer, data_sheet_name)

    headers = df.columns.values.tolist()
    workbook = g_excel_writer.book
    #chart_sheet = workbook.get_worksheet_by_name(sheet_name)
...
def plot_excel_charts2():
    ...
    draw_generic_line_chart(df, SN_IO_PROFILE, 'NFSv3_IOPS', 'Hourly NFSv3 IOPS')
 
    print("===Step 2:")
    file_name_list = get_file_list_in_dir_by_file_name(
        g_one_storage_data_dir_name, 'nfs3.ops_op.txt')
    df = analyze_nfs_generic_breakdown_file_list3(
        file_name_list, NFSV3_OPS_STAT_NAME_DICT)
    draw_nfs_combined_chart2(df, SN_IO_PROFILE, 'NFSv3_BREAKDOWN_BY_OPS',
                             'Hourly NFSv3 IOPS Breakdown by Operation')

    print("===Step 3:")
    file_name_list = get_file_list_in_dir_by_file_name(g_one_storage_data_dir_name, 'nfs4.ops.txt')
    column_name_list = ['date_time', 'iops', 'max', 'min', 'mean']
    df = analyze_generic_file_list(file_name_list, column_name_list)
    draw_generic_line_chart(df, SN_IO_PROFILE, 'NFSv4_IOPS', 'Hourly NFSv4 IOPS')
    
    print("===Step 4:")
    file_name_list = get_file_list_in_dir_by_file_name(
        g_one_storage_data_dir_name, 'nfs4.ops_op.txt')
    df = analyze_nfs_generic_breakdown_file_list3(
        file_name_list, NFSV4_OPS_STAT_NAME_DICT)
    draw_nfs_combined_chart2(df, SN_IO_PROFILE, 'NFSv4_BREAKDOWN_BY_OPS',
...
def analyze_generic_file_list(file_name_list, column_name_list, interval=3600, skiprows=1, header=None):
    ...
    for fn in file_name_list:
        ...
        for i in range(0, int(len(df) / interval)):
            ...
            index += 1
    df_cat = df_cat.sort_values(by=df_cat.columns[0])
...
9               1       create    NaN    NaN    NaN
        ...
        for index, elem in enumerate(non_na_index_list):
            ...
            for other_ops_index in range(elem+1, next_elem):
                row_list[column_name_dict[df.iloc[other_ops_index, 1]]] = df.iloc[other_ops_index, 0]
             
            df_res.loc[df_index] = row_list
            print(row_list)
            
            
#             print(df.iloc[elem, 0] + ' ' + df.iloc[elem, 1])
#             df_res.iloc[df_index, 0] = df.iloc[elem, 0] + ' ' + df.iloc[elem, 1]
#             df_res.iloc[df_index, 1] = df.iloc[elem, 2]
#             df_res.iloc[df_index, column_name_dict[df.iloc[elem, 4]]] = df.iloc[elem, 3]
#             for other_ops_index in range(elem+1, next_elem):
#                 df_res.iloc[column_name_dict[df.iloc[other_ops_index, 1]]] = df.iloc[other_ops_index, 0]
#             
#             print(df.loc[df_index].tolist())
            df_index += 1

        print("final_df_res_start")
        print(df_res)
        print("final_df_res_end")
        for i in range(0, int(len(df_res) / interval)):
            """ date_time column """
            df_cat.loc[index][0] = df_res.iloc[i * interval][0]
            for j in range(1, len(column_name_dict)):
                s = df_res.iloc[i * interval:(i + 1) * interval][j]
                df_cat.loc[index][j] = int(s.mean())
                print("df_cat = " % df_cat)
                #break
            index += 1
...
def analyze_nfs_generic_breakdown_file_list3(file_name_list, column_name_dict, interval=3600):
    df_cat = pd.DataFrame(columns=column_name_dict.keys())
    index = 0
    for fn in file_name_list:
        """ A tricky workaround here to read irregular file """
...
9               1       create    NaN    NaN    NaN
        ...
        """
        df = pd.read_csv(fn, delim_whitespace=True, skiprows=1, low_memory=False)
        first_line = df.columns.tolist()
        df.loc[-1] = first_line
        df.index = df.index + 1
        df = df.sort_index()
        ...
        while index < len(non_na_index_list):
            ...
            if row_list[1] != '0' and df.iloc[elem, 4].isdigit == False:
                print(row_list[1])
                print(df)
                row_list[column_name_dict[df.iloc[elem, 4]]] = df.iloc[elem, 3]
                for other_ops_index in range(elem+1, next_elem):
