...
def cat_all_df(df, column_name_dict, elem, next_elem):
    global g_df_one_file
    one_df = _get_row_dict(df, column_name_dict, elem, next_elem-1)
    g_df_lock.acquire()
    g_df_one_file = g_df_one_file.append(one_df, ignore_index=True)
    g_df_lock.release()
...
9               1       create    NaN    NaN    NaN
        ...
        global g_df_one_file
        g_df_one_file = pd.DataFrame(columns=column_name_dict.keys())
        ...
        while index < len(non_na_index_list):
            ...
            pool.add_task(cat_all_df, df, column_name_dict, elem, next_elem)
...
#             index += interval
        d_print(g_df_one_file, DBG_LEVEL3)
        df_cat = df_cat.append(g_df_one_file)
