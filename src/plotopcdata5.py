...
def plot_excel_charts2():
    file_name_list = traverse_dir(g_one_storage_data_dir_name, 'nfs3.ops.txt')
    print(file_name_list)

#     column_name_list = ['Date', 'Time', 'OPS/SEC', 'OPS/SEC B', 'BREAKDOWN']
...
def traverse_dir(dir_name, file_name):
    file_name_list = []
...
def analyze_generic_file_list(file_name_list, column_name_list, interval=3600, skiprows=1, header=None):
    ...
    print(df_cat)
    #df_cat['DateTime'] = df['Date'].map(str) + ' ' +  df['Time']
    #df_cat['DateTime'] = df['Date'].map(str) + ' ' +  df['Time']
#     df_cat['DateTime'] = df_cat[0].map(str) + ' ' +  df_cat[1]
#     df_res = df_cat[['DateTime', 2]].copy()
